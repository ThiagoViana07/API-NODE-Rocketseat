...
router.get('/', async (req, res) => {
  try {
    const projects = await Project.find(req.body).populate(['user', 'tasks']);
...
router.put('/:projectId', async (req, res) => {
  try {
    ...
    console.log(error)
    res.status(400).send({ error: 'Error update project' })
  }
})
...
router.delete('/:projectId', async (req, res) => {
  try {
    ...
    res.status(400).send({error : 'Error deleting projects'})
  }
})
